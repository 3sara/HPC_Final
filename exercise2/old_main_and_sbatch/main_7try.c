#include "2bcast_tree.c"
#include "2bcast_chain.c"
#include <sys/time.h>
#include <stdlib.h>
#include <stdio.h>
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < WARMUP_ITERATIONS; ++i) {
        binary_tree_broadcast(&dummy_data, my_rank, num_procs, root_rank);
        chain_broadcast(&dummy_data, my_rank, num_procs, root_rank);
    }
    ...
    fprintf(fp, "warmup, barrrier, reduce\n");
    ...
    for (int pow =  1; pow <=  20; ++pow) {
        ...
        if (my_rank == 0) {
            fprintf(fp, "%lld,%f,%f\n", size, total_avg_time_bintree * 1000, total_avg_time_chain * 1000);
        }
    }
    // Chiudi il file CSV
    fclose(fp);
